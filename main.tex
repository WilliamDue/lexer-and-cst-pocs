...
\end{align*}
Where there only exists a single shortests prefix $\alpha$ of $B\gamma$ such that $(y, \alpha, ()) \vdash^* (y', \omega, \pi)$ where $\alpha$ may equal $a$.

Then for the extended grammar $G'$ then the following derivation also exists.
\begin{align*}
  S' \Rightarrow^*_{lm} wu'A_xB'\gamma' \Rightarrow wxB'\gamma' \Rightarrow^* wxy\delta'
\end{align*}
Both of the leftmost derivations will have the common sub derivation $wx$ since they still produce the same language. 

We know $B\gamma$ and $B'\gamma'$ are both produced by leftmost derivations. We also know a leftmost derivation in $G'$ only differs from $G$ by not the need to derive on $A_t$. Therefore substituting $A_t$ with its terminal $t$ would then result in the derivations being the same. So therefore $(B'\gamma')[t_1/A_{t_1}, \dots t_n/A_{t_n}] = B\gamma$. In the case of $\delta'$ here we can just let $\delta'$ be such that $\delta'[t_1/A_{t_1}, \dots t_n/A_{t_n}] = \delta$.

By the assumption we now know there only exists a single shortest prefix $\alpha$ of $B\gamma$ such that $(y, \alpha, ()) \vdash^* (y', \omega, \pi)$.
