...
\end{definition}
\begin{proof}
  If we have a grammar $G$ that is $\LL(k)$. Then for a production $A \to \delta \in P$ and its corresponding production $A \to \delta[A_{t_1}/t_1, \dots A_{t_n}/t_1] = A' \to \delta' \in P_r$ we have.
  \begin{align*}
...
\end{proof}
\noindent Now to show that this extension does not change the grammars that can be parsed we need to be reminded of the PSLS definition.
...
\end{definition}
\begin{proof}
  If we are given an admissible pair $(x, y)$ where $|\PSLS(x, y)| = 1$ for some LL grammar $G$ and some derivation.
  \begin{align*}
  ...
  \end{align*}
  Once again we can use the same argumet, that there is a mapping between a leftmost derivations in $G$ and in $G'$ i.e. $(B_a'\gamma')[t_1/A_{t_1}, \dots t_n/A_{t_n}] = a\gamma$ and $B_a'\gamma' = (a\gamma)[A_{t_1}/t_1, \dots A_{t_n}/t_1]$. Therefore there is a one-to-one mapping between these two derivations. Knowing this and $B_a' \Rightarrow a$ then there only exist a single $a = \text{FIRST}^{G'}_1(B_a')$ for this form of derivation. We would now need to show that $B_a'$ would exist in the first set of the PSLS definition since it cannot exist in the second set. We see the derivation is of the same form as a derivation as in the first set. We also see the shortest prefix $\alpha'$ of $B_a'\gamma'$ is such that $(y, \alpha', ()) \vdash^* (y', \varepsilon, \pi')$ is $B_a'$. By $a = \text{FIRST}^{G'}_1(B_a') = \alpha'$ then $\alpha'$ is the only such prefix.
\end{proof}
