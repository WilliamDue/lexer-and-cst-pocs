...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item If $z[i - 1](q_0)$ maps wrongly to $q_0$ then the traversal of the DFA will be reset and $(z[i - 1] \circ x[i])(q_0)$ will map to the correct state of a new path. This will be the first state after $q_0$ of the next path in the DFA.
      \item If $z[i - 1](q_0)$ maps to the correct state then $(z[i - 1] \circ x[i])(q_0)$ will also map to the correct state.
    \end{itemize}
  \end{itemize}
  Therefore it is known that step 4. will produce the correct paths taken in the DFA.
