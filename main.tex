...
\begin{proof}
  ...
  \end{align*}
  Once again we can use the same argumet, that there is a mapping between a leftmost derivations in $G$ and in $G'$ i.e. $(B_a'\gamma')[t_1/A_{t_1}, \dots t_n/A_{t_n}] = a\gamma$ and $B_a'\gamma' = (a\gamma)[A_{t_1}/t_1, \dots A_{t_n}/t_1]$. Therefore there is a one-to-one mapping between these two derivations. Knowing this and $B_a' \Rightarrow a$ then there only exist a single $a = \text{FIRST}_1(B_a')$ for this form of derivation. We would now need to show that this $B_a'$ would exist in the first set of the PSLS definition since it cannot exist in the second set. We see the derivation is of the same form as a derivation as in the first set. We also see the shortest prefix $\alpha'$ of $B_a'\gamma'$ is such that $(y, \alpha', ()) \vdash^* (y', \omega', \pi')$ is $B_a'$. So $\alpha' = B_a'$.
\end{proof}
